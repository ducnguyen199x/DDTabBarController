...
extension UIView {
    ...
    func equalWidth(toView view: UIView?, multiplier: CGFloat, relatedBy relation: NSLayoutConstraint.Relation = .equal, priority: UILayoutPriority = .required) {
        let constraint = NSLayoutConstraint(item: self, attribute: .width, relatedBy: relation, toItem: view, attribute: .width, multiplier: multiplier, constant: 0)
        constraint.priority = priority
        (superview ?? view)?.addConstraint(constraint)
    }
    
    func equalHeight(toView view: UIView?, multiplier: CGFloat, relatedBy relation: NSLayoutConstraint.Relation = .equal, priority: UILayoutPriority = .required) {
        let constraint = NSLayoutConstraint(item: self, attribute: .height, relatedBy: relation, toItem: view, attribute: .height, multiplier: multiplier, constant: 0)
        constraint.priority = priority
        (superview ?? view)?.addConstraint(constraint)
    }
